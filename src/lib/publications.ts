export const publications = {
	journal: [
		{
			who: [
				'BERNED, G. P.',
				'ROSSI, F. D.',
				'LUIZELLI, M. C.',
				'DE SOUZA, S. X.',
				'BECK, A. C. S.',
				'LORENZON, A. F.'
			],
			where: 'Journal of Systems Architecture , v. x, p., 2020.',
			title: 'Low learning-cost offline strategies for EDP optimization of parallel applications.'
		},
		{
			who: [
				'MEDEIROS, T. S.',
				'BERNED, G. P.',
				'NAVARRO, A.',
				'ROSSI, F. D.',
				'LUIZELLI, M. C.',
				'BRANDALERO, M.',
				'HUBNER, M.',
				'BECK, A. C. S.',
				'LORENZON, A. F.'
			],
			where: 'IEEE Embedded Systems Letters , v. x, p. 1-4, 2020.',
			title: 'Aging-aware Parallel Execution.'
		},
		{
			who: [
				'COUTINHO, D. A. M.',
				'SENSI, D.',
				'LORENZON, A. F.',
				'GEORGIOU, K.',
				'NUNEZ-YANEZ, J.',
				'EDER, K.',
				'XAVIER-DE-SOUZA, S.'
			],
			where: 'Energies , v. 13, p. 1, 2020.',
			title:
				'Performance and Energy Trade-Offs for Parallel Applications on Heterogeneous Multi-Processing.'
		},
		{
			who: [
				'HOHEMBERGER, R.',
				'ROSA, C. E.',
				'PFEIFER, F. R.',
				'ROSA, R. M.',
				'SOUZA, P. S.',
				'LORENZON, A. F.',
				'LUIZELLI, M. C.',
				'ROSSI, F. D.'
			],
			where: 'Measurement , v. 1, p. 1, 2020.',
			title: 'An approach to mitigate challenges to the Electronic Health Records storage.'
		},
		{
			who: [
				'FILHO, R. I. T. C.',
				'LUIZELLI, M. C.',
				'PETRANGELI, S.',
				'VEGA, M. T.',
				'HOOFT, J. V. D.',
				'WAUTERS, T.',
				'TURCK, F.',
				'GASPARY, L. P.'
			],
			where:
				'ACM Transactions on Multimedia Computing Communications and Applications , v. 1, p. 1, 2019.',
			title:
				'Dissecting the Performance of VR Video Streaming Through the VR-EXP Experimentation Platform.'
		},
		{
			who: [
				'JORDAN, M. G.',
				'BRANDALERO, M.',
				'MALFATTI, G. M.',
				'LORENZON, A. F.',
				'SILVA, B. C.',
				'CARRO, L.',
				'BECK, M. R.',
				'BECK, A. C. S.'
			],
			where: 'Design Automation for Embedded Systems, v. 1, p. 1, 2019.',
			title: 'Data clustering for efficient approximate computing.'
		},
		{
			who: [
				'VOGT, F. G.',
				'HOHEMBERGER, R.',
				'MANSILHA, R.',
				'LORENZON, A. F.',
				'ROSSI, F. D.',
				'LUIZELLI, M. C.'
			],
			where: 'IEEE Communications Letters, v. 23, p. 20-25, 2019.',
			title: 'Orchestrating In-Band Data Plane Telemetry with Machine Learning.'
		},
		{
			who: ['HOHEMBERGER, R.', 'LORENZON, A. F.', 'ROSSI, F. D.', 'LUIZELLI, M. C.'],
			where: 'IEEE Communications letters , v. x, p. 1-5, 2019.',
			title: 'Optimizing Distributed Network Monitoring for NFV Service Chains.'
		},
		{
			who: ['MIOTTO, G.', 'LUIZELLI, M. C.', 'CORDEIRO, W. L. C.', 'GASPARY, L. P.'],
			where: 'Journal of Internet Services and Applications, v. 10, p. 3, 2019.',
			title: 'Adaptive placement and chaining of virtual network functions with NFV-PEAR.'
		},
		{
			who: ['LORENZON, A. F.', 'SOUZA, J. D.', 'OLIVEIRA, C. C.', 'BECK, A. C. S.'],
			where: 'IEEE Transactions on Parallel and Distributed Systems, p. 1-16, 2019.',
			title: 'Aurora: Seamless Optimization of OpenMP Applications.'
		}
	],
	conference: [
		{
			who: ['NAVARRO, A. M.', 'LORENZON, A. F.', 'PARRA, E. A.', 'QUEROL, V. B.'],
			where:
				'Euro-Par: 26th International European Conference on Parallel and Distributed Computing, Warsaw, Poland. 2020. p. 1-14.',
			title: 'Optimizing Resource Management through Prediction-based Policies.'
		},
		{
			who: ['HOHEMBERGER, R.', 'LORENZON, A. F.', 'ROSSI, F. D.', 'LUIZELLI, M. C.'],
			where:
				'The 34-th International Conference on Advanced Information Networking and Applications (AINA-2020) Caserta, Italy, 2020. p. 1-8.',
			title:
				'A Heuristic Approach for Large-Scale Orchestration of the In-Band Data Plane Telemetry Problem.'
		},
		{
			who: ['BERNED, G.', 'LUIZELLI, M. C.', 'ROSSI, F. D.', 'BECK, A. C. S.', 'LORENZON, A. F.'],
			where:
				'28th Euromicro International Conference on Parallel, Distributed, and Network-Based Processing (PDP) Sweden, 2020. p. 1-8.',
			title: 'Decreasing the Learning Cost of Offline Parallel Application Optimization Strategies.'
		},
		{
			who: ['HOHEMBERGER, R.', 'LORENZON, A. F.', 'LUIZELLI, M. C.', 'ROSSI, F. D.'],
			where:
				'28th Euromicro International Conference on Parallel, Distributed, and Network-Based Processing (PDP) Sweden, 2020. p. 1-8.',
			title: 'Modeling and Simulating Daily Power Budgets for Sustainable Data Centers.'
		},
		{
			who: [
				'IZOLAN, P.',
				'HOHEMBERGER, R.',
				'KONZEN, M. P.',
				'SAQUETTE, L.',
				'TEMP, D.',
				'LORENZON, A. F',
				'LUIZELLI, M. C.',
				'ROSSI, F. D.'
			],
			where:
				'International Conference on Information Networking (ICOIN), Barcelona – Spain, 2020. p. 1-6.',
			title: 'Low-Cost Fog Computing Platform for Soil Moisture Management.'
		},
		{
			who: [
				'IZOLAN, P.',
				'HOHEMBERGER, R.',
				'KONZEN, M. P.',
				'SAQUETTE, L.',
				'TEMP, D.',
				'LORENZON, A. F',
				'LUIZELLI, M. C.',
				'ROSSI, F. D.'
			],
			where:
				'International Conference on Information Networking (ICOIN), Barcelona – Spain, 2020. p. 1-6.',
			title: 'Towards Elasticity for SME Cloud Services.'
		},
		{
			who: [
				'MEDEIROS, T. S.',
				'ROSSI, F. D.',
				'LUIZELLI, M. C.',
				'BECK, A. C. S.',
				'LORENZON, A. F.'
			],
			where:
				'International Symposium on Computer Architecture and High Performance Computing (SBAC-PAD), Campo Grande – Brazil, 2019. p. 1-6.',
			title: 'Transparent Aging-Aware Thread Throttling.'
		},
		{
			who: [
				'MARQUES, S. M.',
				'MEDEIROS, T. S.',
				'ROSSI, F. D.',
				'LUIZELLI, M. C.',
				'GIRARDI, A.',
				'BECK, A. C. S.',
				'LORENZON, A. F.'
			],
			where:
				'IFIP/IEEE International Conference on Very Large Scale Integration , Cusco-Peru. NY, USA: IEEE, 2019. p. 1-6.',
			title:
				'The Impact of Turbo Frequency on the Energy, Performance, and Aging of Parallel Applications.'
		},
		{
			who: [
				'SOUZA, P.',
				'MARQUES, W.',
				'CONTERATO, M. S.',
				'LUIZELLI, M. C.',
				'LORENZON, A. F.',
				'FERRETO, T. C.',
				'ROSSI, F. D.'
			],
			where:
				'International Symposium on Computers and Communications (ISCC) . NY, USA: IEEE, 2019. p. 1-6.',
			title:
				'Multilevel Resource Allocation for Performance-Aware Energy-Efficient Cloud Data Centers.'
		},
		{
			who: ['CONTERATO, M.', 'FERRETO, T. C.', 'ROSSI, F. D.', 'MARQUES, W. S.', 'SEVERO, P. S.'],
			where:
				'Proceedings of the 34th ACM/SIGAPP Symposium on Applied Computing – SAC ’19. New York: ACM Press, 2019. p. 1384.',
			title:
				'Reducing energy consumption in SDN-based data center networks through flow consolidation strategies.'
		},
		{
			who: [
				'SCHWARZROCK, J.',
				'JORDAN, M. G.',
				'KOROL, G.',
				'OLIVEIRA, C. C.',
				'LORENZON, A. F.',
				'BECK, A. C. S.'
			],
			where:
				'Brazilian Symposium on Computing Systems Engineering (SBESC), Natal-RN. Brazil. NY: IEEE, 2019. p. 1-8.',
			title:
				'On the influence of Data Migration in Dynamic Thread Management of Parallel Applications.'
		},
		{
			who: [
				'VIEIRA, A. N. C.',
				'DE SOUZA, P. S. S.',
				'MARQUES, W.',
				'CONTERATO, M.',
				'FERRETO, T. C.',
				'LUIZELLI, M. C.',
				'LORENZON, A. F.',
				'BECK, A. C. S.',
				'ROSSI, F. D.',
				'NONAKA, J.'
			],
			where: 'IEEE International Symposium on Circuits and Systems (ISCAS). 2019. p. 1-5.',
			title:
				'The Impact of Parallel Programming Interfaces on the Aging of a Multicore Embedded Processor.'
		},
		{
			who: ['BASAT, R. B.', 'EINZIGER, GIL', 'LUIZELLI, M. C.', 'WAISBARD, E.'],
			where: 'IFIP Networking, 2019.',
			title: 'A Black-box Method for Accelerating Measurement Algorithms with Accuracy Guarantees.'
		}
	]
};
